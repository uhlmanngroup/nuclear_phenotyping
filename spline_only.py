...
# np.random.uniform(0,2*np.pi,10)
# x' = x*cos(theta)-y*sin(theta)
# y' = x*sin(theta)+y*cos(theta)

odds = np.arange(0,31,2)
evens = np.arange(1,32,2)
aug_list = []
for theta in np.random.uniform(0,2*np.pi,1):
    ...
    aug_list.append(df_prime)

aug_df = pd.concat(aug_list).set_index("Angle",append=True)
from sklearn.metrics.pairwise import euclidean_distances

# distogram.flatten()
# distogram = sklearn.metrics.pairwise.euclidean_distances(df.iloc[[0]].T)
df_sorted = df.reindex(np.array(sorted(df.columns.astype(np.int))).astype(np.str), axis=1)
df_dist = df_sorted.apply(lambda x: euclidean_distances(np.array(x).reshape(-1, 1)).flatten(),axis=1,result_type="expand")
...
plot = sns.catplot(
    ...
    ci=None,
    hue="Metric",
    data=(df_dist.cellesce.grouped_median("ObjectNumber")
            .cellesce.get_score_report("Cell")
            .assign(**{"Population type": "Organoid"})
            .set_index("Metric")
            .loc[['f1-score', 'recall','precision']]
